...
with st.form("loan_form"):
    ...
    with col2:
        loan_percent_income = st.number_input(
            "Proporsi Pinjaman terhadap Pendapatan (loan_percent_income)",
            min_value=0.00, max_value=2.00, value=round(loan_amnt / max(person_income, 1), 2), step=0.01
        )
...
if submitted:
    df_input = pd.DataFrame([{
        ...
        'person_income_win': person_income,  # diasumsikan income_win = income
    ...
    st.success(f"{label} (Probabilitas: {probability:.2%})") if prediction == 1 else st.error(f"{label} (Probabilitas: {probability:.2%})")
